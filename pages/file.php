                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" 
                                        v-model="form.doc_target_cohorts" 
                                        multiple
                                        size="5"
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" 
                                        v-model="form.doc_target_grades" 
                                        multiple
                                        size="5"
                                        :disabled="form.doc_target_all">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" 
                                        v-model="form.doc_target_classes" 
                                        multiple
                                        size="5"
                                        :disabled="form.doc_target_all">
                            ...
                            <tr v-for="file in filteredFiles" 
                                ...
                                <td class="text-start">
                                    ...
                                    <div v-else>
                                        ...
                                        <div v-if="!file.doc_target_cohorts && !file.doc_target_grades && !file.doc_target_classes" class="text-muted">
                                            未設定
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <button
                                        type="button"
                                        class="btn btn-sm w-100"
                                        :class="Number(file.doc_status) ? 'btn-success' : 'btn-danger'"
...
(() => {
  ...
  // 更可靠的 API 路徑解析
  // 確保在不同環境下（本地、GitHub、不同子目錄）都能正確找到 api.php
  const getApiRoot = () => {
    const pathname = location.pathname;
    
    // 如果路徑包含 /pages/，需要向上找到根目錄的 api.php
    if (pathname.includes('/pages/')) {
      // 找到 /pages/ 的位置，然後向上到根目錄
      const pagesIndex = pathname.indexOf('/pages/');
      const basePath = pathname.substring(0, pagesIndex);
      // 確保路徑以 / 開頭
      return (basePath || '') + '/api.php';
    }
    
    // 如果不在 /pages/ 下，嘗試從當前目錄找
    const lastSlash = pathname.lastIndexOf('/');
    if (lastSlash > 0) {
      return pathname.substring(0, lastSlash) + '/api.php';
    }
  ...
  if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
    console.log('API_ROOT:', API_ROOT, 'Current path:', location.pathname);
  }
