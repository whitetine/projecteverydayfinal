      ...
      <form @submit.prevent="submitForm" id="uploadForm">
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <input type="text" class="form-control" v-model="form.file_name" placeholder="例如：專題指導申請表" required>
          </div>
          <div class="col-md-6">
            <label class="form-label">文件說明</label>
            <input type="text" class="form-control" v-model="form.file_des" placeholder="文件說明（選）">
          </div>
          ...
          <div class="col-md-6">
            ...
            <input type="datetime-local" class="form-control" v-model="form.file_start_d">
            <small class="text-muted">留空表示立即開放</small>
          </div>
          <div class="col-md-6">
            <label class="form-label">截止時間</label>
            <input type="datetime-local" class="form-control" v-model="form.file_end_d">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-12">
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" v-model="form.target_all" id="targetAll"
              ...
              <div class="col-md-4">
                ...
                <select class="form-select" v-model="form.target_cohorts" multiple size="5" :disabled="form.target_all">
                  <option v-for="cohort in cohorts" :value="String(cohort.cohort_ID)">
                    {{ cohort.cohort_name }}
                  </option>
                </select>
              ...
              <div class="col-md-4">
                ...
                <select class="form-select" v-model="form.target_grades" multiple size="5" :disabled="form.target_all">
              ...
              <div class="col-md-4">
                ...
                <select class="form-select" v-model="form.target_classes" multiple size="5" :disabled="form.target_all">
                  <option v-for="classItem in classes" :value="String(classItem.c_ID)">
                    {{ classItem.c_name }}班
                  </option>
                </select>
                <small class="text-muted">可多選，按住 Ctrl/Cmd 鍵</small>
      ...
      <template v-else>
        <div class="table-responsive">
          <table class="table table-bordered table-hover table-sm align-middle mb-0 bg-white text-center">
            ...
            <tbody>
              <tr v-for="file in filteredFiles" :key="file.file_ID" :class="{ 'table-warning': file.is_required }">
                <td>
                  <input type="checkbox" :value="file.file_ID" v-model="selectedFiles">
                </td>
                <td class="text-start">
                ...
                <td>
                  <button type="button" class="btn btn-sm w-100"
                    :class="Number(file.file_status) ? 'btn-success' : 'btn-danger'" @click="toggleStatus(file)">
                    {{ Number(file.file_status) ? '啟用' : '停用' }}
                  </button>
                </td>
                <td>
                  <div class="btn-group" role="group">
                    <button type="button" class="btn btn-sm btn-outline-primary" @click="editFile(file)" title="編輯">
                      <i class="fa-solid fa-edit"></i>
                    </button>
                    <button type="button" class="btn btn-sm btn-outline-danger" @click="deleteFile(file)" title="刪除">
...
<script>
  (() => {
    ...
    const API_ROOT = window.location.origin + '/projecteveryday/api.php';
      ...
      setup() {
        ...
        const fetchFiles = async () => {
          ...
          try {
            ...
            if (!list) {
              console.error("後端資料格式：", data);
              throw new Error('資料格式錯誤（後端未回傳 array）');
            }
        ...
        const submitForm = async () => {
          ...
          fd.append('doc_name', form.value.file_name);
          fd.append('doc_des', form.value.file_des || '');
          fd.append('doc_example', form.value.file); // 這是 PDF
          fd.append('is_required', form.value.is_required ? '1' : '0');
          fd.append('doc_start_d', form.value.file_start_d || '');
          fd.append('doc_end_d', form.value.file_end_d || '');
